...
func (a *API) getShardTables(writer http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		...
		a.respondError(writer, ErrParseRequest, fmt.Sprintf("decode request body failed, cause: %v", err))
		return
	}
	...
	if err != nil {
		...
		a.respondError(writer, ErrGetCluster, fmt.Sprintf("get cluster failed, clusterName: %s, cause: %v", getShardTables.ClusterName, err))
		return
	}
...
func (a *API) transferLeader(writer http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		...
		a.respondError(writer, ErrGetCluster, fmt.Sprintf("get cluster failed, clusterName: %s, cause: %v", transferLeaderRequest.ClusterName, err))
		return
	}
	...
	if err != nil {
		...
		a.respondError(writer, ErrCreateProcedure, fmt.Sprintf("create transfer leader procedure failed, cause: %v", err))
		return
	}
	...
	if err != nil {
		...
		a.respondError(writer, ErrSubmitProcedure, fmt.Sprintf("submit transfer leader procedure failed, cause: %v", err))
		return
	}
...
func (a *API) route(writer http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		...
		a.respondError(writer, ErrForwardToLeader, fmt.Sprintf("forward to leader failed, cause: %v", err))
		return
	}
	...
	if err != nil {
		...
		a.respondError(writer, ErrRouteTable, fmt.Sprintf("route tables failed, cause: %v", err))
		return
	}
...
func (a *API) getNodeShards(writer http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		...
		a.respondError(writer, ErrGetNodeShards, fmt.Sprintf("get node shards failed, cause: %v", err))
		return
	}
...
func (a *API) dropTable(writer http.ResponseWriter, req *http.Request) {
	...
	if err := a.clusterManager.DropTable(context.Background(), dropTableRequest.ClusterName, dropTableRequest.SchemaName, dropTableRequest.Table); err != nil {
		...
		a.respondError(writer, ErrDropTable, fmt.Sprintf("drop table failed, cause: %v", err))
		return
	}
...
func (a *API) split(writer http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		...
		a.respondError(writer, ErrGetCluster, fmt.Sprintf("get cluster failed, clusterName: %s, cause: %v", splitRequest.ClusterName, err))
		return
	}
	...
	if err != nil {
		...
		a.respondError(writer, ErrAllocShardID, fmt.Sprintf("alloc shard id failed, cause: %v", err))
		return
	}
	...
	if err != nil {
		...
		a.respondError(writer, ErrCreateProcedure, fmt.Sprintf("create split procedure failed, cause: %v", err))
		return
	}

	if err := c.GetProcedureManager().Submit(ctx, splitProcedure); err != nil {
		log.Error("submit split procedure failed", zap.Error(err))
		a.respondError(writer, ErrSubmitProcedure, fmt.Sprintf("submit split procedure failed, cause: %v", err))
		return
	}
...
func (a *API) listClusters(writer http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		...
		a.respondError(writer, ErrGetCluster, fmt.Sprintf("list clusters failed, cause: %v", err))
		return
	}
...
func (a *API) createCluster(writer http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		...
		a.respondError(writer, ErrParseTopology, fmt.Sprintf("parse topology type failed, cause: %v", err))
		return
	}
	...
	if err != nil {
		...
		a.respondError(writer, metadata.ErrCreateCluster, fmt.Sprintf("create cluster failed, cause: %v", err))
		return
	}
...
func (a *API) updateCluster(writer http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		...
		a.respondError(writer, ErrGetCluster, fmt.Sprintf("get cluster failed, clusterName: %s, cause: %v", updateClusterRequest.ClusterName, err))
		return
	}
	...
	if err := a.clusterManager.UpdateCluster(req.Context(), updateClusterRequest.ClusterName, metadata.UpdateClusterOpts{
		...
		a.respondError(writer, metadata.ErrUpdateCluster, fmt.Sprintf("update cluster failed, cause: %v", err))
		return
	}
